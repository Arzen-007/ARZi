...
from ARZi.models import Challenges, Teams, Users, db
from ARZi.utils import get_app_config, get_config, set_config
from ARZi.utils.config import is_setup
from ARZi.utils.crypto import sha256


def update_check(force=False):
    """
    Makes a request to arzi.io to check if there is a new version of ARZi available. The service is provided in return
    ...
    if update:
        try:
            ...
            check = requests.get(
                "https://versioning.arzi.io/check", params=params, timeout=3
            ).json()
