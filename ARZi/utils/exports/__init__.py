...
from ARZi import __version__ as ARZI_VERSION
from ARZi.cache import cache
from ARZi.constants.themes import DEFAULT_THEME
from ARZi.models import db, get_class_by_tablename
from ARZi.plugins import get_plugin_names
from ARZi.plugins.migrations import current as plugin_current
from ARZi.plugins.migrations import upgrade as plugin_upgrade
from ARZi.utils import get_app_config, set_config, string_types
from ARZi.utils.dates import unix_time
from ARZi.utils.exports.databases import is_database_mariadb
from ARZi.utils.exports.freeze import freeze_export
from ARZi.utils.migrations import (
    create_database,
    drop_database,
...
from ARZi.utils.uploads import get_uploader
...
def import_ctf(backup, erase=True):
    ...
    if "db" not in member_dirs:
        ...
        raise Exception(
            'ARZi couldn\'t find the "db" folder in this backup. '
    ...
    # Check if the alembic version is from ARZi 1.x
    if alembic_version in (
        ...
        set_import_error(
            "Exception: The version of ARZi that this backup is from is too old to be automatically imported."
        )
        raise Exception(
            "The version of ARZi that this backup is from is too old to be automatically imported."
        )
    ...
    if sqlite is False and alembic_version not in get_available_revisions():
        set_import_error(
            "Exception: The target migration in this backup is not available in this version of ARZi."
        )
        raise Exception(
            "The target migration in this backup is not available in this version of ARZi."
        )
            ...
            if member.startswith("db/"):
                ...
                if data:
                    ...
                    for i, entry in enumerate(saved["results"]):
                        ...
                        # databases. Specifically between MySQL and MariaDB. Because ARZi standardizes against MySQL
    ...
    set_config("ctf_version", ARZI_VERSION)
