...
from ARZi.utils.decorators import admins_only as admins_only_wrapper
from ARZi.utils.plugins import override_template as utils_override_template
...
from ARZi.utils.plugins import register_script as utils_register_plugin_script
from ARZi.utils.plugins import register_stylesheet as utils_register_plugin_stylesheet
...
def register_plugin_asset(app, asset_path, admins_only=False, endpoint=None):
    """
    Registers an file path to be served by ARZi
...
def bypass_csrf_protection(f):
    ...
    :return: Returns a function with the _bypass_csrf attribute set which tells ARZi to not require CSRF protection.
    """
    f._bypass_csrf = True
    return f
...
def init_plugins(app):
    """
    Searches for the load function in modules in the ARZi/plugins folder. This function is called with the current ARZi
    app as a parameter. This allows ARZi plugins to modify ARZi's behavior.
    ...
    if app.config.get("SAFE_MODE", False) is False:
        for plugin in get_plugin_names():
            module = "." + plugin
            module = importlib.import_module(module, package="ARZi.plugins")
            module.load(app)
