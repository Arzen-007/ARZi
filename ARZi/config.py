...
def gen_secret_key():
    ...
    try:
        with open(".arzi_secret_key", "rb") as secret:
            key = secret.read()
    except OSError:
        key = None
    ...
    if not key:
        ...
        try:
            with open(".arzi_secret_key", "wb") as secret:
                secret.write(key)
                secret.flush()
        except OSError:
            pass
...
class ServerConfig(object):
    ...
    if not DATABASE_URL:
        if empty_str_cast(config_ini["server"]["DATABASE_HOST"]) is not None:
            ...
            DATABASE_URL = str(URL(
                ...
                username=empty_str_cast(config_ini["server"]["DATABASE_USER"]) or "arzi",
                ...
                database=empty_str_cast(config_ini["server"]["DATABASE_NAME"]) or "arzi",
            ))
        else:
            # default to local SQLite DB
            DATABASE_URL = f"sqlite:///{os.path.dirname(os.path.abspath(__file__))}/arzi.db"
    ...
    """
    TRUSTED_PROXIES:
    Defines a set of regular expressions used for finding a user's IP address if the ARZi instance
    ...
    ARZi only uses IP addresses for cursory tracking purposes. It is ill-advised to do anything complicated based
