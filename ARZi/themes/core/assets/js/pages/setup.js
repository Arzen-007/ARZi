...
function mlcSetup(_event) {
  let params = {
    ...
    event_url: window.location.origin + ARZi.config.urlRoot,
    redirect_url: window.location.origin + ARZi.config.urlRoot + "/redirect",
    integration_setup_url:
      window.location.origin + ARZi.config.urlRoot + "/setup/integrations",
...
$(() => {
  ...
  $("#setup-form").submit(function(e) {
    if ($("#newsletter-checkbox").prop("checked")) {
      ...
      $.ajax({
        url:
          "https://newsletters.arzi.io/lists/ot889gr1sa0e1/subscribe/post-json?c=?",
        data: {
          email: email,
