...
const displayChal = chal => {
  return Promise.all([
    ARZi.api.get_challenge({ challengeId: chal.id }),
    $.getScript(config.urlRoot + chal.script),
    $.get(config.urlRoot + chal.template)
  ]).then(responses => {
    const challenge = ARZi._internal.challenge;
...
function renderSubmissionResponse(response) {
  ...
  const next_btn = $(
    ...
    setTimeout(function() {
      loadChal(ARZi._internal.challenge.data.next_id);
    }, 500);
  });
  ...
  if (result.status === "authentication_required") {
    ...
    if (ARZi._internal.challenge.data.next_id) {
...
function getSolves(id) {
  return ARZi.api.get_challenge_solves({ challengeId: id }).then(response => {
...
function loadChals() {
  return ARZi.api.get_challenge_list().then(function(response) {
    const categories = [];
...
const displayUnlock = id => {
  ezQuery({
    ...
    success: () => {
      ...
      ARZi.api.post_unlock_list({}, params).then(response => {
        if (response.success) {
          ARZi.api.get_hint({ hintId: id }).then(response => {
