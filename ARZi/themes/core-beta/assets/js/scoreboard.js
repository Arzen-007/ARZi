...
Alpine.data("ScoreboardDetail", () => ({
  ...
  async update() {
    this.data = await ARZi.pages.scoreboard.getScoreboardDetail(10, this.activeBracket);

    let optionMerge = window.scoreboardChartOptions;
    let option = getOption(ARZi.config.userMode, this.data, optionMerge);
...
Alpine.data("ScoreboardList", () => ({
  ...
  async update() {
    this.brackets = await ARZi.pages.scoreboard.getBrackets(ARZi.config.userMode);
    this.standings = await ARZi.pages.scoreboard.getScoreboard();
  },
