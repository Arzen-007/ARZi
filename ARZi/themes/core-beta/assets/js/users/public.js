...
Alpine.data("UserGraphs", () => ({
  ...
  async init() {
    this.solves = await ARZi.pages.users.userSolves(window.USER.id);
    this.fails = await ARZi.pages.users.userFails(window.USER.id);
    this.awards = await ARZi.pages.users.userAwards(window.USER.id);
