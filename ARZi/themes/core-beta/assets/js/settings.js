...
Alpine.data("SettingsForm", () => ({
  ...
  async updateProfile() {
    ...
    // Send API request
    const response = await ARZi.pages.settings.updateSettings(data);
    if (response.success) {
      this.success = true;
      this.error = false;
...
Alpine.data("TokensForm", () => ({
  ...
  async generateToken() {
    ...
    const response = await ARZi.pages.settings.generateToken(data);
...
Alpine.data("Tokens", () => ({
  ...
  async deleteSelectedToken() {
    await ARZi.pages.settings.deleteToken(this.selectedTokenId);
