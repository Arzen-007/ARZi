...
Alpine.data("Hint", () => ({
  ...
  async showHint(event) {
    if (event.target.open) {
      let response = await ARZi.pages.challenge.loadHint(this.id);

      // Hint has some kind of prerequisite or access prevention
      if (response.errors) {
        event.target.open = false;
        ARZi._functions.challenge.displayUnlockError(response);
        return;
      }
      ...
      if (hint.content) {
        ...
        let answer = await ARZi.pages.challenge.displayUnlock(this.id);
        if (answer) {
          let unlock = await ARZi.pages.challenge.loadUnlock(this.id);
          ...
          if (unlock.success) {
            ...
            event.target.open = false;
            ARZi._functions.challenge.displayUnlockError(unlock);
          }
        } else {
          event.target.open = false;
        }
...
Alpine.data("Challenge", () => ({
  ...
  getStyles() {
    ...
    try {
      let size = ARZi.config.themeSettings.challenge_window_size;
      switch (size) {
        case "sm":
  ...
  async showSolves() {
    this.solves = await ARZi.pages.challenge.loadSolves(this.id);
  ...
  async showSubmissions() {
    let response = await ARZi.pages.users.userSubmissions("me", this.id);
  ...
  async showSolution() {
    ...
    ARZi._functions.challenge.displaySolution = solution => {
      this.solution = solution.html;
      new Tab(this.$el).show();
    };
    await ARZi.pages.challenge.displaySolution(solution_id);
  },
  ...
  async getShareUrl() {
    ...
    const response = await ARZi.fetch("/api/v1/shares", {
      method: "POST",
  ...
  async submitChallenge() {
    this.response = await ARZi.pages.challenge.submitChallenge(
      this.id,
      this.submission,
    );
...
Alpine.data("ChallengeBoard", () => ({
  ...
  async init() {
    this.challenges = await ARZi.pages.challenges.getChallenges();
    this.loaded = true;
  ...
  getCategories() {
    ...
    try {
      const f = ARZi.config.themeSettings.challenge_category_order;
      if (f) {
  ...
  getChallenges(category) {
    ...
    try {
      const f = ARZi.config.themeSettings.challenge_order;
      if (f) {
        const getSort = new Function(`return (${f})`);
  ...
  async loadChallenge(challengeId) {
    await ARZi.pages.challenge.displayChallenge(challengeId, challenge => {
