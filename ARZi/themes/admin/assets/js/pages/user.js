  ...
  ARZi.fetch(url, {
    ...
    .then(function (response) {
      if (response.success) {
        ...
        window.location = ARZi.config.urlRoot + "/admin/users/" + user_id;
      } else {
...
function updateUser(event) {
  ...
  ARZi.fetch("/api/v1/users/" + window.USER_ID, {
    method: "PATCH",
    credentials: "same-origin",
    headers: {
      Accept: "application/json",
  ...
  ezQuery({
    ...
    success: function () {
      ARZi.fetch("/api/v1/users/" + window.USER_ID, {
        ...
        .then(function (response) {
          if (response.success) {
            window.location = ARZi.config.urlRoot + "/admin/users";
          }
        });
    },
  });
...
function emailUser(event) {
  ...
  ARZi.fetch("/api/v1/users/" + window.USER_ID + "/email", {
    method: "POST",
    credentials: "same-origin",
    headers: {
      Accept: "application/json",
