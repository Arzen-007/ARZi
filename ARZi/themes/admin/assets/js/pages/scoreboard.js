...
const api_func = {
  users: (x, y) => ARZi.api.patch_user_public({ userId: x }, y),
  teams: (x, y) => ARZi.api.patch_team_public({ teamId: x }, y),
};
...
function toggleAccount() {
  ...
  api_func[ARZi.config.userMode](id, params).then((response) => {
    if (response.success) {
      if (hidden) {
...
function toggleSelectedAccounts(selectedAccounts, action) {
  ...
  for (let accId of selectedAccounts.accounts) {
    reqs.push(api_func[ARZi.config.userMode](accId, params));
  }
