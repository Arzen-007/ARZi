...
const graph_configs = {
  "#solves-graph": {
    data: () => ARZi.api.get_challenge_solve_statistics(),
    format: (response) => {
  ...
  "#keys-pie-graph": {
    data: () => ARZi.api.get_submission_property_counts({ column: "type" }),
    format: (response) => {
      const data = response.data;
  ...
  "#categories-pie-graph": {
    data: () => ARZi.api.get_challenge_property_counts({ column: "category" }),
    format: (response) => {
      const data = response.data;
  ...
  "#solve-percentages-graph": {
    layout: (annotations) => ({
      ...
      yaxis: {
        title: `Percentage of ${
          ARZi.config.userMode.charAt(0).toUpperCase() +
          ARZi.config.userMode.slice(1)
        } (%)`,
        range: [0, 100],
      },
      annotations: annotations,
    }),
    data: () => ARZi.api.get_challenge_solve_percentages(),
    format: (response) => {
      const data = response.data;
  ...
  "#score-distribution-graph": {
    ...
    data: () =>
      ARZi.fetch("/api/v1/statistics/scores/distribution").then(
        function (response) {
          return response.json();
        },
      ),
