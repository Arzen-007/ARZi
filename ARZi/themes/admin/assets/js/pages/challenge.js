...
function handleChallengeOptions(event) {
  ...
  let save_challenge = function () {
    ARZi.fetch("/api/v1/challenges/" + params.challenge_id, {
      method: "PATCH",
      credentials: "same-origin",
      headers: {
        Accept: "application/json",
      ...
      .then(function (data) {
        if (data.success) {
          setTimeout(function () {
            window.location =
              ARZi.config.urlRoot + "/admin/challenges/" + params.challenge_id;
          }, 700);
        }
      });
  };
...
$(() => {
  $(".preview-challenge").click(function (_e) {
    let url = `${ARZi.config.urlRoot}/admin/challenges/preview/${window.CHALLENGE_ID}`;
  ...
  $(".delete-challenge").click(function (_e) {
    ezQuery({
      ...
      success: function () {
        ARZi.fetch("/api/v1/challenges/" + window.CHALLENGE_ID, {
          method: "DELETE",
        })
          .then(function (response) {
            return response.json();
          ...
          .then(function (response) {
            if (response.success) {
              window.location = ARZi.config.urlRoot + "/admin/challenges";
            }
          });
      },
    });
  ...
  $("#challenge-update-container > form").submit(function (e) {
    ...
    ARZi.fetch("/api/v1/challenges/" + window.CHALLENGE_ID + "/flags", {
      method: "GET",
      credentials: "same-origin",
      headers: {
        Accept: "application/json",
  ...
  $.get(ARZi.config.urlRoot + "/api/v1/challenges/types", function (response) {
