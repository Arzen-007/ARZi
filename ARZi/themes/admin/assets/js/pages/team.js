...
import { htmlEntities } from "@arziio/arzi-js/utils/html";
  ...
  ARZi.fetch("/api/v1/teams", {
    ...
    .then(function (response) {
      if (response.success) {
        ...
        window.location = ARZi.config.urlRoot + "/admin/teams/" + team_id;
      } else {
...
function updateTeam(event) {
  ...
  ARZi.fetch("/api/v1/teams/" + window.TEAM_ID, {
    method: "PATCH",
    credentials: "same-origin",
    headers: {
      Accept: "application/json",
  ...
  ezQuery({
    ...
    success: function () {
      ...
      for (var subId of submissionIDs) {
        let req = ARZi.fetch(`/api/v1/submissions/${subId}`, {
          method: "PATCH",
          credentials: "same-origin",
          headers: {
            Accept: "application/json",
...
function deleteSelectedSubmissions(event, target) {
  ...
  ezQuery({
    ...
    success: function () {
      const reqs = [];
      for (var subId of submissionIDs) {
        reqs.push(ARZi.api.delete_submission({ submissionId: subId }));
      }
      Promise.all(reqs).then((_responses) => {
        window.location.reload();
      });
  ...
  ezQuery({
    ...
    success: function () {
      ...
      for (var awardID of awardIDs) {
        let req = ARZi.fetch("/api/v1/awards/" + awardID, {
          method: "DELETE",
          credentials: "same-origin",
          headers: {
            Accept: "application/json",
      ...
      ezAlert({
        ...
        success: function () {
          ...
          for (var challengeID of challengeIDs) {
            ...
            let req = ARZi.fetch("/api/v1/submissions", {
              method: "POST",
              credentials: "same-origin",
              headers: {
                Accept: "application/json",
...
const api_funcs = {
  team: [
    (x) => ARZi.api.get_team_solves({ teamId: x }),
    (x) => ARZi.api.get_team_fails({ teamId: x }),
    (x) => ARZi.api.get_team_awards({ teamId: x }),
  ],
  user: [
    (x) => ARZi.api.get_user_solves({ userId: x }),
    (x) => ARZi.api.get_user_fails({ userId: x }),
    (x) => ARZi.api.get_user_awards({ userId: x }),
  ],
};
...
$(() => {
  ...
  $(".invite-team").click(function (_e) {
    ARZi.fetch(`/api/v1/teams/${window.TEAM_ID}/members`, {
      method: "POST",
      credentials: "same-origin",
      headers: {
        Accept: "application/json",
      ...
      .then(function (response) {
        if (response.success) {
          ...
          let url = `${window.location.origin}${ARZi.config.urlRoot}/teams/invite?code=${code}`;
    ...
    ezQuery({
      ...
      success: function () {
        ARZi.fetch("/api/v1/teams/" + window.TEAM_ID, {
          ...
          .then(function (response) {
            if (response.success) {
              window.location = ARZi.config.urlRoot + "/admin/teams";
            }
          });
      },
    });
