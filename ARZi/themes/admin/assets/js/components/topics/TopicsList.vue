...
export default {
  ...
  methods: {
    loadTopics: function () {
      ARZi.fetch(`/api/v1/challenges/${this.$props.challenge_id}/topics`, {
        method: "GET",
    ...
    searchTopics: function () {
      ...
      ARZi.fetch(`/api/v1/topics?field=value&q=${this.topicValue}`, {
        method: "GET",
        credentials: "same-origin",
        headers: {
          Accept: "application/json",
    ...
    deleteTopic: function (topic_id) {
      ARZi.fetch(`/api/v1/topics?type=challenge&target_id=${topic_id}`, {
        method: "DELETE",
      })
        .then((response) => {
          return response.json();
