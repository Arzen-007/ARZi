...
export default {
  ...
  methods: {
    searchUsers: function () {
      ...
      ARZi.fetch(`/api/v1/users?view=admin&field=name&q=${this.searchedName}`, {
        method: "GET",
    ...
    handleAddUsersRequest: function () {
      ...
      this.selectedUsers.forEach((user) => {
        ...
        reqs.push(
          ARZi.fetch(`/api/v1/teams/${this.$props.team_id}/members`, {
            method: "POST",
            credentials: "same-origin",
            headers: {
              Accept: "application/json",
    ...
    handleRemoveUsersFromTeams: function () {
      ...
      this.selectedUsers.forEach((user) => {
        let body = { user_id: user.id };
        reqs.push(
          ARZi.fetch(`/api/v1/teams/${user.team_id}/members`, {
            method: "DELETE",
            body: JSON.stringify(body),
          }),
        );
