...
import ARZi.utils.config
from ARZi import utils
from ARZi.constants.themes import ADMIN_THEME, DEFAULT_THEME
from ARZi.plugins import init_plugins
from ARZi.utils.crypto import sha256
from ARZi.utils.initialization import (
    init_cli,
    init_events,
    init_logs,
...
from ARZi.utils.migrations import create_database, migrations, stamp_latest_revision
from ARZi.utils.sessions import CachingSessionInterface
from ARZi.utils.updates import update_check
...
class ARZiFlask(Flask):
    def __init__(self, *args, **kwargs):
        ...
        self.request_class = ARZiRequest
    ...
    def create_jinja_environment(self):
        ...
        return super(ARZiFlask, self).create_jinja_environment()
    ...
    def create_url_adapter(self, request):
        ...
        return super(ARZiFlask, self).create_url_adapter(request)
...
def confirm_upgrade():
    if sys.stdin.isatty():
        print("/*\\ ARZi has updated and must update the database! /*\\")
        print("/*\\ Please backup your database before proceeding! /*\\")
        print("/*\\ ARZi maintainers are not responsible for any data loss! /*\\")
...
def create_app(config="ARZi.config.Config"):
    app = ARZiFlask(__name__)
    with app.app_context():
        app.config.from_object(config)

        from ARZi.cache import cache
        from ARZi.utils import import_in_progress
        ...
        while import_in_progress():
            print("Import currently in progress, ARZi startup paused for 5 seconds")
            time.sleep(5)
        ...
        for theme_name in ARZi.utils.config.get_themes():
        ...
        from ARZi.models import (  # noqa: F401
            Challenges,
            Fails,
            Files,
            Flags,
        ...
        # Upgrading from an older version of ARZi
        ...
        from ARZi.api import api
        from ARZi.auth import auth
        from ARZi.challenges import challenges
        from ARZi.errors import render_error
        from ARZi.events import events
        from ARZi.scoreboard import scoreboard
        from ARZi.share import social
        from ARZi.teams import teams
        from ARZi.users import users
        from ARZi.views import views
