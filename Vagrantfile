...
commands="CREATE DATABASE arzi;
CREATE USER 'arziuser'@'localhost' IDENTIFIED BY 'arzi';
GRANT USAGE ON *.* TO 'arziuser'@'localhost' IDENTIFIED BY 'arzi';
GRANT ALL privileges ON arzi.* TO 'arziuser'@'localhost';FLUSH PRIVILEGES;"
echo "${commands}" | sudo /usr/bin/mysql -u root -parzi
SCRIPT
...
export DATABASE_URL="mysql+pymysql://arziuser:arzi@localhost/arzi"

cd /vagrant
python manage.py db upgrade

echo "Starting ARZi"
tmux new-session -d -n "arzi" -c "/vagrant" -s "arzi" "gunicorn --bind 0.0.0.0:8000 -w 4 'ARZi:create_app()'"
SCRIPT
