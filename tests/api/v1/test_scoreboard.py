...
def test_scoreboard_is_cached():
    ...
    with app.app_context():
        ...
        saved = app.cache.get("ARZi.utils.scoreboard.get_scoreboard_detail_memver")
        ...
        with login_as_user(app, "user1") as client:
            ...
            assert (
                app.cache.get("ARZi.utils.scoreboard.get_scoreboard_detail_memver")
                == saved
            )
            cached = client.get("/api/v1/scoreboard/top/10").get_json()
            assert cached["data"] == orig
            assert app.cache.get("ARZi.utils.scoreboard.get_scoreboard_detail_memver")
            ...
            new = app.cache.get("ARZi.utils.scoreboard.get_scoreboard_detail_memver")
            assert new != saved
            assert (
                app.cache.get(make_template_fragment_key("public_scoreboard_table"))
                is None
...
def test_scoreboard_tie_break_ordering_with_awards_under_teams():
    ...
    """
    app = create_arzi(user_mode="teams")
    with app.app_context():
