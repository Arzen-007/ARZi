...
from tests.helpers import create_arzi, destroy_arzi, gen_user, login_as_user
...
def test_that_arzi_can_be_deployed_in_subdir():
    """Test that ARZi can be deployed in a subdirectory"""
    ...
    app = create_arzi(setup=False, application_root="/ctf")
    with app.app_context():
        with app.test_client() as client:
            r = client.get("/")
...
def test_that_request_path_hijacking_works_properly():
    """Test that the ARZiRequest subclass correctly mimics the Flask Request when it should"""
    app = create_arzi(setup=False, application_root="/ctf")
    assert app.request_class.__name__ == "ARZiRequest"
    with app.app_context():
...
def test_theme_fallback_config():
    ...
    app = create_arzi(config=ThemeFallbackConfig)
    # Make an empty theme
    try:
