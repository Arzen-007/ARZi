...
from ARZi.models import Challenges, Flags, Teams, Users
from ARZi.utils import text_type
from ARZi.utils.exports import export_ctf, import_ctf
...
def test_export_ctf():
    """Test that ARZi can export the database"""
    app = create_arzi()
...
def test_import_ctf():
    """Test that ARZi can import a CTF"""
    app = create_arzi()
    if not app.config.get("SQLALCHEMY_DATABASE_URI").startswith("sqlite"):
        with app.app_context():
            base_user = "user"
