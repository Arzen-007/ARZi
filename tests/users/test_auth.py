...
from ARZi.models import Users, db
...
def test_user_can_confirm_email(mock_smtp):
    ...
    with app.app_context(), freeze_time("2012-01-14 03:21:34"):
        # Set ARZi to only allow confirmed users and send emails
        set_config("verify_emails", True)
        set_config("mail_server", "localhost")
        set_config("mail_port", 25)
        set_config("mail_useauth", True)
...
def test_user_can_reset_password(mock_smtp):
    ...
    with app.app_context():
        ...
        with app.test_client() as client:
            ...
            # Build the email
            msg = (
                "Did you initiate a password reset on ARZi? If you didn't initiate this request you can ignore this email. "
